function [V,nmax] = calcLatticeCoulombOptimized(meshstruct,k,nmax,E)
%CALL:      V = calcLatticeCoulombOptimized(meshstruct,k,nmax,E)
%DESCRIPTION: Calculates the lattice sum
%   VE = ( sum_(R=Rall) exp(i*k*R)/|r-R| )*exp(-i*k*dr)
...
expiGnmr = exp(-1i*( bsxfun(@times, p(:,1), Gnm(:,1).') + bsxfun(@times, p(:,2), Gnm(:,2).') ) );
...
for j = 1:Ntri
    ...
    V(:,t(j,:)) = V(:,t(j,:)) + ( GaussQuadMat * ... 
                   ( funV1(cpnt,k,Gnm,expiGnmr,erfcabskGnm,A_uc) + ...
                     funV2(p,cpnt,k,Rnm,expiRnmk,expikr,E) ) ).'*s;
end

...
if any(out)
   ...
   %%% d = dsearch(p(:,1),p(:,2),t,pi(out,1),pi(out,2)); [THE FUNCTION 'dsearch' HAS BEEN REMOVED FROM MATLAB; 
   %%% fi(out) = f(d);                                    WE USE THE FOLLOWING INSTEAD (Thomas Christensen, Sept. 21, 2016)]
   DT = delaunayTriangulation(p);
   d = nearestNeighbor(DT,pi(out,:));
   fi(out) = f(d);    
end

...
%                    completion { TRUE }.
%   options.plot   : Determines whether mesh is plotted (requires also that
%                    options.output = TRUE) { TRUE }                   [TC]
%   options.edgerefine : Allows (TRUE) or forbids (FALSE) the refinement of
%                        edge geometry by the insertion of nodes via
%                        splitting. { TRUE }                           [TC]
%
% EXAMPLE:
%
...
% Discretise edges
pbnd = boundarynodes(qtree.p,qtree.t,qtree.h,node,edge,options.output,options.edgerefine);
...
if options.output && (isfield(options,'plot') && options.plot)
   figure('Name','Mesh')
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function p = boundarynodes(ph,th,hh,node,edge,output,edgerefine)
...
i = tsearchn(ph,th,p);   %[EDIT 2016 (Thomas Christensen)]
            %tsearch was removed from Matlab, so this line needs fixing
            %following the suggestion by Mike (23 Sep 2013) in the link:
            %www.mathworks.com/matlabcentral/fileexchange/25555-mesh2d-automatic-mesh-generation            
h = tinterp(ph,th,hh,p,i);
    ...
    while true
        ...
        ratio = L./he;
        split = (ratio>=1.5);
        if edgerefine == 1 && any(split)  %New points are only allowed to be added to the boundary if edgerefine == 0;
            % Split edge at midpoint      %otherwise, we forbid it. [EDIT 19 May, 2016 (Thomas Christensen)])
            n1 = e(split,1);
            n2 = e(split,2);
            pm = 0.5*(p(n1,:)+p(n2,:));
...
if ~isempty(options)
   ...
   if isfield(options,'plot')                                              % Plot on/off independent of output being false
       checklogicalscalar(options.plot,'options.plot');
   else
       options.plot = d_plot;
   end
   if isfield(options,'edgerefine')                                        % Edge split allowed on/off
       checklogicalscalar(options.plot,'options.plot');
   else
       options.edgerefine = d_edgerefine;
   end
else                                                                       % Default values
   options.mlim   = d_mlim;
   options.maxit  = d_maxit;

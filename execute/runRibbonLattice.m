function runRibbonLattice(latticetype,numcell,Ns,Ncirc,adivd,CoulombMethod,Nk,numeigs)
...
for kk = Nk:-1:1
    ...
    switch CoulombMethod
        ...
        case '2Dsupercell'
            ...
            nmax(2,1) = max(ceil(2*E*erfccut/norm(blochmesh.G{1},2)) + (norm(kvec(kk,:)) == 0)*1,2);
            nmax(2,2) = max(ceil(2*E*erfccut/norm(blochmesh.G{2},2)) + (norm(kvec(kk,:)) == 0)*1,2);
            
            W = calcLatticeCoulombOptimized(blochmesh,kvec(kk,:),nmax,E);
    end
    [DRi,DL] = calcLatticeDifferentialIsotropic(blochmesh,kvec(kk,:));
    [zeta(:,kk),eigrho(:,:,kk)] = calcEigen(W,DL,DRi,numeigs,[],true);
    eigphi(:,:,kk) = W*eigrho(:,:,kk);

...
Ne = 100;
blochmesh = geomPeriodicInclusion('square','disk',Ne,'const');
R = blochmesh.R;
G = calcReciprocal(R);
...
for kk = 1:size(kvec,1);
    [zeta(:,kk),~,~,~,~,ene_eV(:,kk)] = calcLatticeEigen(blochmesh,kvec(kk,:),[],L,sigma,1);
...
export_fig(['DispersionExtendedGraphene_Ne' num2str(Ne)],'-pdf')
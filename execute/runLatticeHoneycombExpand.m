function savepath = runLatticeHoneycombExpand(kspec,Ns,Ncirc,adivd,expandshrink,Nk,savephi,models,addstr)
...
%% SETUP DEFAULTS
if nargin < 1 || ~exist('kspec','var') || isempty(kspec)
    kspec = 'irrfbz'; %Number of points along each unit cell side
end
if nargin < 2 || ~exist('Ns','var') || isempty(Ns)
    Ns = 230; %Number of points along each unit cell side
end
if nargin < 3 || ~exist('Ncirc','var') || isempty(Ncirc)
    Ncirc = 41; %Number of points used in circular inclusion
end
if nargin < 4 || ~exist('adivd','var') || isempty(adivd)
    adivd = 1.5*sqrt(3)*5/2; %Period divided by inclusion diameter
end
if nargin < 5 || ~exist('expandshrink','var') || isempty(expandshrink)
...
fprintf('\n|----- SETUP -----|\n')
fprintf([strf('kspec','s'), strf('Ns','g'), strf('Ncirc','g'), strf('adivd','g'), strf('expandshrink','g'), strf('Nk','s')              , strf('savephi','g'), strf('addstr','s')],...
         kspecstr         , Ns            , Ncirc            , adivd            , expandshrink,             strrep(num2str(Nk),'  ','x'), savephi            , addstr)
...
savename = ['lattice_expandedhoneycomb_' strrep(strrep(num2str(expandshrink*100,'%+g'),'-','m'),'+','p') '_adivd' num2str(adivd) '_' kspecstr '_Ns' num2str(Ns) '_Ncirc' num2str(Ncirc) '_savephi' num2str(savephi) addstr];

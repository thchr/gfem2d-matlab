function savepath = runLatticeHoneycomb(latticetype,kspec,Ns,Ncirc,adivd,Nk,savephi,models,addstr,dottype)
...
if nargin < 2 || ~exist('kspec','var') || isempty(kspec)
    kspec = 'projx'; %Number of points along each unit cell side
end
...
if nargin < 5 || ~exist('adivd','var') || isempty(adivd)
    adivd = 3.5; %Period divided by inclusion diameter
end
...
if nargin < 9 || ~exist('addstr','var') || isempty(addstr)
    addstr = []; %Whether to add a modifier string to the savename ([] = nothing added)
elseif ~isempty(addstr)
    addstr = ['_' addstr];
end 
if nargin < 10 || ~exist('dottype','var') || isempty(dottype)
    dottype = ''; %Whether to add a modifier string to the savename ([] = nothing added)
end 
...
fprintf('\n|----- SETUP -----|\n')
fprintf([strf('lattice type','s'), strf('kspec','s'), strf('Ns','g'), strf('Ncirc','g'), strf('adivd','g'), strf('Nk','s')              , strf('savephi','g'), strf('addstr','s')],...
         latticetype             , kspecstr         , Ns            , Ncirc            , adivd            , strrep(num2str(Nk),'  ','x'), savephi            , addstr)
...
inclusion = {bsxfun(@plus,hole,-shift),bsxfun(@plus,hole,shift)};
if strcmpi(dottype,'antidot') || strcmpi(dottype,'')
    ...
    dottype = ''; %Don't include in savename (implicitly assumed)
elseif strcmpi(dottype,'dot')
    [mesh.p,mesh.t] = geomPolygon(inclusion,2.5/Ncirc,1);
    mesh.area = meshArea(mesh.p,mesh.t);
    mesh.R = {[1,0],[cosd(60),sind(60)]};
    mesh.remesh = struct('p',mesh.p,'t',mesh.t);
    Ns = NaN;    dottype = '_dot_';     %Prepare for use in savename
end
...
savename = ['lattice_honeycomb' dottype '_adivd' num2str(adivd) '_' kspecstr '_Ns' num2str(Ns) '_Ncirc' num2str(Ncirc) '_savephi' num2str(savephi) addstr];

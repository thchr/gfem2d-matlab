function [blochmesh,origmesh,boundary] = geomRibbonDistinctInclusions(R,rep2,inclusions,cut2,Nuc,hdatafun,showmesh)
...
for ii = 1:numel(inclusions)
    newinclusion = unique(inclusions{ii},'rows','stable');
    if numel(newinclusion) ~= numel(inclusions{ii})  %Warn if duplicates found
        inclusions{ii} = newinclusion;
...
shiftbreaks = [shiftincl(floor(max(rep2)/2)),max(shiftincl)]
if ~isempty(inclusions)
    for aa = 1:numel(shiftincl)
        nodesincl{aa} = bsxfun(@plus,inclusions{find(shiftincl(aa)<=shiftbreaks,1)}, shiftincl(aa)*R{2});
    end
